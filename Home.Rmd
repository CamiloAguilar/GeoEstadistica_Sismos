...
### Magnitud Sísmica para Antioquia (Escala de richter)
#### Febrero de 2018
##### Realizado por:

- **Nestor Jardany Serrano Rojas** <https://github.com/jardanys/GeoEstadistica_Sismos>
- **Camilo Alejandro Aguilar Laverde** <https://github.com/CamiloAguilar/Estadistica_Espacial>
- **Alvaro Enrique Villaba Mendez** <alvarovillalba@usantotomas.edu.co>
...
El objetivo del estudio es estimar y predecir la actividad sísmica de la magnitud local en la escala de Richter. Los  rangos espaciales donde puede haber movimientos sismicos dependiendo de la escala de medición. 
...
La información utilizada en el siguiente documento se toma de Red Sismológica Nacional de Colombia <http://200.119.88.135/RSNC/>, especificamente del BOLETÍN PRELIMINAR DE EVENTO SÍSMICO. Los datos usados son para el departamento de Antioquia, toda la actividad sismica en Junio de 2017. (123 observaciones, en la siguiente tabla se observan las primeras 6 observaciones).
...
A continuación se muestran una descripción de los datos, aparentemente los datos no se diferencian mucho de una distribución normal y se decide continuar sin transformar los archivos. Si se desea normalizar los datos se pueden transformar con alguna función matemática.
...
El siguiente mapa espacial muestra la actividad sismica en el departamento de Antioquia, Colombia. La zona occidental muestra una mayor actividad, en la zona centro la actividad es casi nula. La magnitud más fuerte es de 3.5 en la escala de Ricther en el mes de Junio de 2017.
...
Se realizan los gráficos de la variable **Magnitud en escala de Richter** contra las direcciones (Longitud, Latitud). Se observan con cierta estacionariedad y sin tendencia significativa. En los gráficos la media de la maginitud local es constante sobre la región de observación, por tanto no es necesario remover esta dependencia.
...
El semivariogra se realiza utilizando las coordenadas y la magnitud. Se convierte en geodata y se revisan que no existen coordenadas duplicadas. 
...
La siguiente gráfica muestra el ajuste de los modelos especificados anteriormente por función exponencial.
...
Se realiza una validación cruzada de los modelos descritos y se escoge el modelo 3. Como el modelo de mejor ajuste.
...
Se utiliza el método de **Krigign Universal** para predecir dentro del territorio de Antioquia las zonas con mayor posibilidad sísmita de acuerdo a la magnitud en la escala de Richter **actividad sísmitca**. Tomando una función cuadratica de las distancias.
...
Como se puede observar en las sigueintes graficas:

- Kriging Universal para la Magnitud Local
- Mapa para las varianzas de Magnitud Local
...
Se utiliza el método de **Krigign Ordinario**, modelando solo con la magnitud para predecir dentro del territorio de Antioquia. Las zonas con mayor posibilidad sismita de acuerdo a la magnitud en la escala de Richter (actividad sismitca) es sur oriental, se observa diferencias respecto al **Kriging Universal**, para **Kriging Ordinario** toda la zona occidental del departamento presentaría alta posibilidad de sismos con mayor magnitud. 
...
- Kriging Ordinario para la Magnitud Local
- Mapa para las varianzas de Magnitud Local

La varianza es alto y presenta islas, la zona occidental de departamento es más propenso a tener actividad sismica con mayor nivel de magnitud y la zona oriental es la de menor actividad. La varianza es muy alta para la zona occidental y centro del departamento.
